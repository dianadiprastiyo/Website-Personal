...
include('vendor/CircleToPolygon.php');

$query_kantor = "SELECT * FROM kantor WHERE nama='lokasi'";
$result_kantor = $connection->query($query_kantor);
if ($result_kantor->num_rows > 0) {
  $row  = $result_kantor->fetch_assoc();

  $coordinates = [$row['setting_2'], $row['setting_1']]; // [lon, lat]
  $radius = $row['setting_3'];                           // in meters
  $numberOfEdges = 4;                    // optional that defaults to 32
...
$geofence->addPoint(new Coordinate($result['coordinates'][0][1][1], $result['coordinates'][0][1][0]));
$geofence->addPoint(new Coordinate($result['coordinates'][0][2][1], $result['coordinates'][0][2][0]));
$geofence->addPoint(new Coordinate($result['coordinates'][0][3][1], $result['coordinates'][0][3][0]));
$geofence->addPoint(new Coordinate($result['coordinates'][0][4][1], $result['coordinates'][0][4][0]));
...
switch (@$_GET['action']) {
  ...
  case 'registrasi':
    ...
    if (empty($error)) {
      ...
      $pesan .= '[Detail Akun] :';
      $pesan .= 'Nama : ' . $employees_name . '<br>Email : ' . $employees_email . '<br>Password: ' . $password_send . '<br>IP : ' . $ip_login . '<br>Browser : ' . $browser . '';
      $pesan .= 'Hormat Kami,<br>' . $site_name . '<br>Email otomatis, Mohon tidak membalas email ini"';
      $pesan .= "</body></html>";
      $to     = $employees_email;
  ...
  case 'absent':
    ...
    if (empty($_POST['qrcode'])) {
      $error['qr'] = 'QR tidak boleh kosong';
    } else {
      $qrcode = mysqli_real_escape_string($connection, $_POST['qrcode']);
    }

    if (empty($_POST['latitude'])) {
      $error['lokasi'] = 'Lokasi tidak boleh kosong';
    } else {
      $latitude = mysqli_real_escape_string($connection, $_POST['latitude']);
    }

    if (empty($error)) {
      $lokasi = explode(',', $latitude);
      $lat =  $lokasi[0];
      $lon =  $lokasi[1];
      $Point = new Coordinate($lat, $lon);
      if ($geofence->contains($Point) == false) {
        echo 'Anda berada di luar kantor, Absen GAGAL!';
      } else if ($geofence->contains($Point) == true) {
        ...
        $query_u = "SELECT employees.id,employees.employees_code,employees.employees_name,employees.shift_id,shift.shift_id,shift.time_in,shift.time_out FROM employees,shift WHERE employees.shift_id=shift.shift_id AND employees.employees_code='$qrcode' AND employees.id='$row_user[id]'";
        ...
        echo 'Error Geofencing!';
      }
    } else {
      if ($error['qr']) {
        echo  $error['qr'];
      } else if ($error['lokasi']) {
